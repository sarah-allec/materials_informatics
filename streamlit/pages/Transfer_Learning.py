...
y_true_no_tl_lv_metals = no_tl_pred_df.y_true
y_pred_no_tl_lv_metals = no_tl_pred_df.y_pred
...
fig = px.scatter( x = y_true_no_tl_lv_metals, y = y_pred_no_tl_lv_metals, color_discrete_sequence=["deepskyblue"] )
fig.add_trace( go.Scatter(x=y_true_no_tl_lv_metals, y=y_true_no_tl_lv_metals,
...
#st.write("I now utilize the outputs of a pre-trained model trained on theoretical band gaps. This dataset is much larger than the experimental band gap dataset, so we expect that the dataset is more diverse and will learn a more general relationship between composition and band gap. The parity plot below shows the accuracy improvement as a result of transfer learning, where the yellow circles are the transfer-learned predictions and the sky blue circles are the original (non-transfer-learned) predictions. In particular, the coefficient of determination for non-metal band gap prediction has now increased from 0.70 in the original model to 0.87 and the mean absolute error for non-metal band gap prediction has decreased from 0.5 eV to 0.4 eV.")
#
#fig = px.scatter( data_frame = expt_gap_df_cpy, x = 'gap expt', y = 'gap pred', color = 'metal type', color_discrete_sequence=["#FDFD96", "violet"] )
#fig.add_trace( go.Scatter( x = expt_gap_df['gap expt'], y = expt_gap_df['gap pred'], mode='markers',
#               marker=dict(color='deepskyblue'), name='original' ) )
#
#fig.add_trace( go.Scatter(x=expt_gap_df['gap expt'], y=expt_gap_df['gap expt'],
#                    mode='lines', line = dict(shape = 'linear', color = '#C1E1C1', dash = 'dash'),
#                    showlegend=False, name='Parity line') )
#fig.update_layout(
#    xaxis_title="Actual band gap (eV)", yaxis_title="Predicted band gap (eV)"
